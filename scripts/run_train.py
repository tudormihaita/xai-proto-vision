...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--use-bbox-crop", dest="use_bbox_crop", action="store_true",
                   help="crop images to annotated object bounding boxes before transforms")
...
def build_trainer(args, model: nn.Module) -> Trainer:
    ...
    if args.method == "protopnet":
        ...
        return ProtoPNetTrainer(
            ...
            # Paper's iterative schedule (Chen et al. main.py): push every 10
            # joint epochs (and always at the end), each push followed by 20
            # last-layer convex-optimisation iterations. StepLR(step=5,gamma=0.1)
            # decays the joint LR so post-push steps stay small and the cycle
...
def main() -> None:
    ...
    train_loader = DataLoader(
        load_dataset(args.dataset, "train", use_bbox_crop=args.use_bbox_crop),
        shuffle=True,
        **loader_kwargs,
    )
    val_loader = DataLoader(
        load_dataset(args.dataset, "val", use_bbox_crop=args.use_bbox_crop),
        shuffle=False,
        **loader_kwargs,
    )
    test_loader = DataLoader(
        load_dataset(args.dataset, "test", use_bbox_crop=args.use_bbox_crop),
        shuffle=False,
        **loader_kwargs,
    )
    push_loader = None
    if args.method == "protopnet":
        push_ds = load_dataset(args.dataset, "train", use_bbox_crop=args.use_bbox_crop)
        # Push must use clean deterministic views and stable source indices.
